...
class MCPAgent:
    ...
    def _get_tools_description(self) -> str:
        """Get a description of all available MCP tools for the LLM."""
    ...
    async def process_user_request(self, user_message: str) -> str:
        """Process user request using Gemini 2.5 Flash to decide which MCP tools to call."""
        try:
            # Create prompt for Gemini to decide which tools to call
            system_prompt = f"""
You are an intelligent MCP (Model Context Protocol) agent that helps users with hotel reservation operations.
...
"""
    ...
    async def _format_reservation_response(self, raw_json_response: str) -> str:
        ...
        try:
            formatting_prompt = f"""
You are a hotel concierge assistant. Convert this reservation API response into a friendly, natural language summary for hotel staff.
