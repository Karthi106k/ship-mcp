"""Fixed Gemini API client for chat functionality."""
...
class GeminiClientFixed:
    """Fixed client for interacting with Google's Gemini API."""
    
    def __init__(self):
        self.settings = get_settings()
        genai.configure(api_key=self.settings.gemini_api_key)
        self.model = genai.GenerativeModel('gemini-2.5-flash')
        logger.info("GeminiClientFixed initialized with Gemini 2.5 Flash")
    ...
    async def generate_response(
        ...
        try:
            logger.info("GeminiClientFixed: Generating response with fixed implementation")
            ...
            if messages and len(messages) > 0:
                ...
                logger.info("GeminiClientFixed: Using conversation format")
            else:
                # Just use the prompt directly for extraction tasks
                gemini_prompt = prompt
                logger.info("GeminiClientFixed: Using direct prompt format")
            
            # Generate response
            response = await self.model.generate_content_async(gemini_prompt)
            
            result = response.text
            logger.info(f"GeminiClientFixed: Response generated successfully, length: {len(result)}")
            return result
            
        except Exception as e:
            logger.error(f"GeminiClientFixed: Error generating response: {e}")
